...
class SycopeApi:
    ...
    def log_out(self) -> requests.Response:
        repsonse = self.session.get(self.host + "/npm/api/v1/logout", verify=False)
        return repsonse
    ...
    def get_lookups(self) -> list:
        ...
        r = self.session.get(
            self.host
            + self.api_endpoint
            + '/npm/api/v1/config-elements?offset=0&limit=2147483647&filter=category = "lookup.lookup"',
            verify=False,
        )
        all_data = r.json()["data"]
        if isinstance(all_data, list):
    ...
    def get_lookup(self, lookup_name: str) -> tuple[str, dict]:
        ...
        if lookup_id:
            r = self.session.get(
                self.host + self.api_endpoint + f"/npm/api/v1/config-element-lookup/csvFile/{lookup_id}",
                verify=False,
            )
    ...
    def remove_index(
        ...
        if id_to_remove:
            ...
            r = self.session.delete(
                self.host + self.api_endpoint + "/npm/api/v1/config-element-index/user-index/" + index_id,
                verify=False,
            )
